...
def process_dialog(df, lower=True, text_separator=" "):
    ...
    def text_normalize(text: str) -> str:
        ...
        text = re.sub("its a", "it's a", text)
        text = re.sub("haven t", "haven't", text)
        text = re.sub("i - i", "I", text)
        text = re.sub("wouldnt", "wouldn't", text)
        text = re.sub("didnt", "didn't", text)
        text = re.sub(" hed ", " he'd ", text)
        text = re.sub("^im ", "I'm ", text)
        text = re.sub(" im ", " I'm ", text)
        text = re.sub("7he", "the", text)
        text = re.sub("5h", "sh", text)
        text = re.sub("5e", "se", text)
        text = re.sub("7hat", "that", text)
